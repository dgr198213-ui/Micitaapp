...
| V-18 | `staff_public_read` y `services_public_read` filtraban solo por `active`, sin acotar por `business_id`: un anónimo podía volcar el nombre de **todo** el personal, o el catálogo y precios de **todos** los servicios, de **toda** la plataforma en una sola consulta REST sin filtro | Alta | `0015_v18_scoped_storefront.sql`: nueva función `get_business_storefront(slug)` (mismo patrón que `get_available_slots`/`create_public_booking`) — devuelve solo el negocio pedido. `SELECT` directo revocado de `anon` en `staff` y `services`; `/api/b/[slug]` y `/b/[slug]` (server component) reescritos para usar la función |
| V-21 | `cancelAppointmentAction` no comprobaba si su `.update()` afectó alguna fila: si RLS lo bloqueaba (p. ej. un staff cancelando una cita que no es suya), la acción devolvía éxito igualmente, registraba un evento de cancelación y marcaba `notification_jobs` como cancelados para una cita que en realidad **nunca cambió de estado** | Media | Revisadas todas las Server Actions con el mismo patrón (`.update()` sin comprobar filas afectadas): `cancelAppointmentAction`, `updateCustomerNotes`, `anonymizeCustomer`, `setServiceActive`, `setStaffActive`. Todas usan ahora `.select("id")` tras el `update` y lanzan `FORBIDDEN` si no hay filas — `updateService` ya era segura porque `.select().single()` lanza por sí solo con 0 filas |
