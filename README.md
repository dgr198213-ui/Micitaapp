...
| V-02 | La política RLS de `memberships` acotaba qué negocio administra el owner, pero no qué rol puede escribir: un owner podía auto-promocionarse a `platform_admin` | Crítica | Política reescrita: excluye `platform_admin` tanto en lectura como en escritura del propio owner |
...
| V-08 | Sin `RESEND_API_KEY`, el job se marcaba `sent` aunque no se enviara nada | Alta | En producción lanza excepción → el job cae a `failed` y dispara la alerta de cola envejecida |
